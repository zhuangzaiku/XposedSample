package com.github.zzk.xposed.utils;
...
import com.github.zzk.xposed.Main;
...
public class SearchClasses {
    ...
    public static boolean loadConfig(XC_LoadPackage.LoadPackageParam lpPram, String curVersionName) {
        try {
            SharedPreferences pref = getPreferenceInstance();
            HookParams hp = new Gson().fromJson(pref.getString("params", ""), HookParams.class);
            if(hp == null || !hp.versionName.equals(curVersionName) || hp.versionCode != HookParams.VERSION_CODE) {
                return  false;
            }

            HookParams.setInstance(hp);
            XposedBridge.log("load config success");
            return true;
        } catch (Error | Exception e) {
            XposedBridge.log(e);
        }
    ...
    private static XSharedPreferences getPreferenceInstance() {
        if(preferencesInstance == null) {
            preferencesInstance = new XSharedPreferences(Main.class.getPackage().getName(), HookParams.WECHAT_HOOK_CONFIG_NAME);
